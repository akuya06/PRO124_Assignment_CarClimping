...
public class ShopDemo1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Xóa hết các item có sẵn trong ScrollView trước khi thêm item mới
        foreach (Transform child in ShopScrollView)
        {
            if (child != ItemTemplate) // Không xóa mẫu item
    ...
    private void UpdateItemUI(GameObject item, ShopItem shopItem, int index)
    {
        ...
        purchaseButton.onClick.RemoveAllListeners(); // Xóa các sự kiện cũ nếu có
        purchaseButton.onClick.AddListener(() => OnItemPurchased(index)); // Đảm bảo chỉ có 1 sự kiện
    }
